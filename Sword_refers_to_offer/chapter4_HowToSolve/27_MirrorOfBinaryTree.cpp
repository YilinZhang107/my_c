...
void MirrorRecursively(BinaryTreeNode *pNode){
    if(pNode == nullptr)
        return;
    BinaryTreeNode *pT = pNode->m_pLeft;
...
void MirrorIteratively(BinaryTreeNode *pNode){
    ...
    while(!nodes.empty()){
        BinaryTreeNode *pParent = nodes.top();
        nodes.pop();

        BinaryTreeNode *pT = pParent->m_pLeft;
        pParent->m_pLeft = pParent->m_pRight;
        pParent->m_pRight = pT;
        if(pParent->m_pLeft)
            nodes.push(pParent->m_pLeft);
        if(pParent->m_pRight)
            nodes.push(pParent->m_pRight);
    }
}
