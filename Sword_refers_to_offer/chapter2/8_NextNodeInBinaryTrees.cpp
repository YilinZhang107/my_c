...
BinaryTreeNode* GetNext(BinaryTreeNode* pNode){
    ...
    if(pNode->m_pRight != nullptr){
        ...
        //如果是它父亲的左子节点, 则next是它父亲
        if(pNode->m_pParent && pNode->m_pParent->m_pLeft == pNode){
            pNext = pNode->m_pParent;
        }
        //是父亲的右子节点,向上找到第一个是左节点的父亲
        else{
            while(pNode->m_pParent && pNode->m_pParent->m_pRight == pNode){
                pNode = pNode->m_pParent;
            }
            pNext = pNode->m_pParent;
        }
        /*更好的写法, 上面的if else是同一类
        while(pNode->m_pParent && pNode->m_pParent->m_pRight==pNode){
            pNode = pNode->m_pParent;
        }
        pNext = pNode->m_pParent; //因为不存在父节点时,pParent=nullptr
        */
    }
    return pNext;
}
