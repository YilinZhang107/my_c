...
//自己写的,很乱,不太好
bool HasSubtree_my(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2){
    if(pRoot2 == nullptr)
        return true;
    if(pRoot1 == nullptr)
        return false;
    ...
    if(pRoot1->m_nValue == pRoot2->m_nValue){
        if(pRoot2->m_pLeft == nullptr && pRoot2->m_pRight == nullptr)
            ...
            if(HasSubtree_my(pRoot1->m_pLeft,pRoot2->m_pLeft) && HasSubtree_my(pRoot1->m_pRight, pRoot2->m_pRight))
                return true;
    }
    if(HasSubtree_my(pRoot1->m_pLeft,pRoot2) || HasSubtree_my(pRoot1->m_pRight,pRoot2))
        return true;
    return false;   
}
...
bool HasSubtree(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2){
    bool result = false;
    if(pRoot1 != nullptr && pRoot2 != nullptr){//Root2为空时算不存在
        if(pRoot1->m_nValue == pRoot2->m_nValue)//节点值相等时
